...
```text
IFRP-FRPC-V1:<加密内容>
bash <(curl -fsSL 'https://raw.githubusercontent.com/qimaoww/install-frp/main/frp.sh') --import-frps-code 'IFRP-FRPC-V1:...' '解密码'
```
...
```text
IFRP-XTCP-V1:<加密内容>
bash <(curl -fsSL 'https://raw.githubusercontent.com/qimaoww/install-frp/main/frp.sh') --import-xtcp-code 'IFRP-XTCP-V1:...' '解密码'
```
...
旧配置可以直接在 XTCP 菜单选择“检查/修复现有配置”。脚本会扫描默认 `frpc.d` 或命名实例 `frpc.d` 里的 XTCP 配置，先显示摘要，再备份并把协议保持/改为 `quic`，保留 `keepTunnelOpen = true`，把 `fallbackTimeoutMs` 调到 5000，并补齐 `disableAssistedAddrs`。
