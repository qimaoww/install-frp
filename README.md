...
被访问端会写入 `[[proxies]] type = "stcp"`，并输出加密导入码、解密码和一键导入命令。已有 STCP 被访问端配置也可以选择“复制接入码”重新导出：
...
已有 XTCP 被访问端配置可以选择“复制接入码”重新导出。旧配置可以直接在 XTCP 菜单选择“检查/修复”。脚本会扫描当前客户端 `frpc.d` 里的 XTCP 配置，先显示摘要，再备份并把协议保持/改为 `quic`，保留 `keepTunnelOpen = true`，把 `fallbackTimeoutMs` 调到 5000，并按你的选择启用或移除 `disableAssistedAddrs`。
